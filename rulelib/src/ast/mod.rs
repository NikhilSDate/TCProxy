...
*
* ## Description:
* The goal of this module is to 'clean up' the parse tree generated by PEST into something more usable.
*
* ## Public Interface:
* To use this module, you are expected to invoke `AstNode::try_from` on a PEST parse tree where the
* root node is a `Rule::Program`.
* Any other input is undefined behavior.
*
* The output will be a `Result<AstNode, AstParseError>`, where on success, the returned `AstNode`
* will be of variant `AstNode::Program`.
*
* ## Developer Notes:
* We co-opt `std::convert::TryFrom`, which is the idiomatic way to perform safe comparisons in Rust;
* see: https://doc.rust-lang.org/std/convert/trait.TryFrom.html
*
* We violate the interface internally to make the code a bit cleaner; namely, through recursive descent;
...
*  - If the current node is a special case of a terminal, then we immediately return the matching case to our parent.
